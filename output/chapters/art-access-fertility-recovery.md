...
**In plain terms: ART is a rework line at the end of a production process, and the question is how many units it actually saves.**

Imagine a factory that has been missing its output target for years. Late in the process someone installs a rework line: units that would have been scrapped go through it, and some come out sellable. At the end of the year you count the units that came off the rework line and report that number as the line's contribution to output.

Two things are wrong with that number, and both are the whole of this chapter.

The first is that **some of those units would have been fixed anyway.** Not all scrap is final; some of it gets repaired downstream, or turns out on inspection not to have been defective. Counting everything that passes through the rework line as *saved by* the rework line assumes the alternative was zero, and nobody checked.

The second is subtler and worse. **The rework line may have changed behaviour upstream.** If everyone on the floor knows there is a repair stage, the pressure to get it right the first time falls. Some of the scrap the line is repairing is scrap it caused. A line that repairs damage it partly created is contributing less than its throughput suggests — and possibly than zero, if the induced carelessness exceeds the repair rate.

Fertility postponement is the upstream behaviour. ART is the rework line. This chapter asks what the line is worth, and finds that the literature which counts its throughput and the literature which measures the alternative are two different literatures that cite each other and do not connect.
...
### 5.1 The naive estimator in this literature, and what it conditions on
...
### 6.3 The behavioural channel, which is better measured than expected and still weak
...
Per phenomenon, not per chapter. **Rated by one rater**; PROTOCOL §5 requires three independent raters and that requirement is open (§10).
...
4. **The demographic-significance denominator** — ART births over all births, or ART's contribution to the TFR. §7 shows these differ materially because ART concentrates at older ages. A ruling is needed and A.12's arm must use the same one.
...
1. **Job A1, the counterfactual set — 2 of 14.** Everything in §6.1 and §9 is conditional on it. Eleven of the twelve missing are blocked-but-open: a browser, not a proxy.

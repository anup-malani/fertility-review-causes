...
|--------|-------|-------|--------------------|---------------|---------|
| [TICK-087](TICK-087-stigma-reduction-contraception-abortion.md) | A.6 Reduction in Stigma Around Contraception and Abortion — **PARKED 2026-09-17 before stage 2.** `404` found the selecting frame omits the registry's own construct ("legitimation"): 668 → 1,225 corrected, so A.6 is not the smallest and A.3 took its place as TICK-088. Hypothesis still live and unstarted; the diagnostic is inheritable. **Claimed LOCALLY ONLY — the push failed on expired credentials, so `ticket.sh`'s `ls-remote` check cannot see this claim** | Shravan | `087-stigma-reduction-contraception-abortion` | 2026-09-17T18:57:58Z | `literature/search-logs/a6-term-diagnostics-*`, `source/build/goldset/404_a6_term_diagnostics.py` |

...
4. **Implication:** def-1's correction answers 'is the query biased toward keyword-sourced papers?' (here: no). To ALSO bound the worst-case vocabulary ceiling, report the adversarial (vocab-disconnected) subset recall as a secondary number — the def-3 probe discussed at design.

#!/usr/bin/env python3
"""
90_a3_tier_ab_frame.py — A.3 (diffusion and social-learning of fertility control), stage A4.

Build the two-tier gold FRAME from the A3 cold-start anchors (step 89; GACS channels 1 + 3). Direct
mirror of D.3.b's `73_d3b_tier_ab_frame.py` and B.1's `65_b1_tier_ab_frame.py`; only the SLUG, cache
namespace, and cell taxonomy change.

  Tier A  = the direct-empirical seeds (the anchors themselves), resolved in OpenAlex and enriched with
            abstracts. This is the primary empirical recall set. Theory-canon anchors (the ideational /
            social-learning theory and the Princeton EFP synthesis seeds) and the six routing decoys are
            split out, NOT counted as Tier-A empirical.
...
Neither tier is frozen here. This is a candidate frame, mirroring child-labor step 57, B.1 step 65, and
D.3.b step 73.

FORWARD-SEED POLICY (inherited): forward-cite an anchor UNLESS it is a routing decoy OR
  (cited_by_count > FWD_THEORY_CAP AND provisional_cell is a THEORY cell). Backward references from ALL
  resolved anchors. Excluded forward seeds are logged with counts. On the 2026-09-18 run the cap did
  NOT bind: OpenAlex's counts put even the most-cited theory anchor (Bongaarts-Watkins 1996) at 1,048,
  below the 1,500 cap, so all 14 non-decoy anchors were forward-seeded and the frame stayed manageable
  at ~3.5k. The cap is retained as a guard against a runaway theory cloud, not because one is expected.

WALL STRUCTURE (channel-excluded A.3, resolved 2026-09-18): the six routing decoys (A.20 channel ×2,
  A.2 technology, A.19 vertical, A.5 program, D.1 value) are backward/forward context but NOT Tier-A
  empirical and are NOT forward-seeded. They must surface as route-away at the later screen.

LEAKAGE WALL (carried from step 89): the EFP/NAS synthesis volumes that fed channel-1 anchors do NOT
  feed A.3 production-query terms. This step only builds the citation frame; it mines no query vocabulary.
...
# A.3 theory cells: formal diffusion/social-learning theory + the Princeton EFP descriptive canon.
# Neither is a Tier-A empirical seed; both are forward-capped (their citation clouds are huge).
THEORY_CELLS = {"DIFFUSION_THEORY", "PRINCETON_CANON"}
# A.3 empirical cells: social-exposure/peer, spatial diffusion, cultural-boundary content, and the
# legitimation-spread cell. OFF_* cells are routing decoys, excluded by the ROUTING_DECOY family test.
EMPIRICAL_CELLS = {"PRIMARY_SOCIAL_EXPOSURE", "PRIMARY_SPATIAL_DIFFUSION",
...
def resolve(anchor, refresh=False):
    ...
    # wrongly sink a correct DOI match below the floor (the book/subtitle low-J cases).
    if anchor.get("doi"):
        try:
...
def main():
    ...
    L += ["", "## Next gate", "",
          "Screen the whole Tier-B frame with the A.3 topical/estimand rubric, routing on the six walls "
          "(vs A.20 channels, A.2 technology, A.6 stigma-level, A.19 vertical transmission, A.5 program "
          "supply, D.1 value shift). Do NOT prune the frame by vocabulary distance from the future "
          "production query — that would bias Recall(B). The six routing decoys (La Ferrara-Chong-Duryea "
          "and Jensen-Oster -> A.20; Goldin-Katz -> A.2; Fernandez-Fogli -> A.19; Miller -> A.5; "
          "Lesthaeghe -> D.1) must surface as route-away at screen."]

#!/usr/bin/env python3
"""
93_d2c_make_screen_batches.py — D.2.c (son preference), stage-3 screen input.

Prepare the merged ~4,035-record screen frame (92) for blinded title/abstract LLM screening. No record is
...
Mirror of 93_a14 / C.2.h step 56. SLUG, SEED, and the rubric (D.2.c's cells, its SIX walls, its
PM/FDT/SDT scope, and the reverse-causality + substitution + mortality identification tags) differ. The
routing decoys must surface route-away: Angrist 2002 -> OFF_ADULT_SEX_RATIO; Pop-Eleches 2006 ->
OFF_ABORTION_GENERAL; Knodel 1987 -> OFF_PARITY_STOPPING; Nugent 1985 -> OFF_OLD_AGE_SECURITY;
Jejeebhoy 1995 -> OFF_GENDER_EQUITY; Preston 1978 -> OFF_CHILD_MORTALITY.
...
RUBRIC = """# Blinded title/abstract screening rubric — son preference and gender-biased fertility norms (D.2.c) — v1

#!/usr/bin/env python3
"""
56_c2h_make_screen_batches.py — C.2.h (digital leisure substitution), stage-3 screen input.

Prepare the merged 2,546-record screen frame (55) for blinded title/abstract LLM screening. No record
...
Mirror of A.3 step 91. Only SLUG, SEED, id-field (`id`, not `paperId`), and the rubric (C.2.h's cells,
its FIVE walls, and the reverse-causality identification tag) differ. The five wall decoys must surface
route-away: Bloom et al. -> OFF_WAGE_C2E; Rosenfeld et al. -> OFF_DATINGAPP_A24; Bongaarts 1978 ->
OFF_COITAL_A14; Lesthaeghe -> OFF_VALUES_D1A; Twenge et al. 2018 -> OFF_MENTALHEALTH_D3A.
...
RUBRIC = """# Blinded title/abstract screening rubric — digital leisure substitution (C.2.h) — v1
...
def main():
    ...
    committed = {"slug": SLUG, "stage": "blinded_title_abstract_screen_input",
                 ...
                 "blinded_fields": ["doi", "cited_by_count", "source_channels"],

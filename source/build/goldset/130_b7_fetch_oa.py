#!/usr/bin/env python3
"""
130_b7_fetch_oa.py — B.7, stage 5. Fetch the openly-available full texts and extract their text.

Reads the OA status file written by 129 and fetches every record marked open, into the house path
...
def main():
    ...
    for wid, st in oa.items():
        ...
        job = "A_primary" if wid in gate and gate[wid]["cell"].startswith("PRIMARY_") else (
              "B_held" if wid in gate else "C_parameter")
        ...
        if os.path.exists(txt) and os.path.getsize(txt) > 500:
            rows.append([wid, doi, job, st["oa_status"], st.get("host", ""), "OK/OK",
        ...
        rows.append([wid, doi, job, st["oa_status"], st.get("host", ""),
                     f"{fstat}/{tstat}" if tstat else fstat, fnote, title[:90]])

    with open(OUT_LOG, "w", newline="") as fh:
        w = csv.writer(fh)
        w.writerow(["openalex_id", "doi", "job", "oa_status", "host", "result", "note", "title"])
        w.writerows(rows)

    got = sum(1 for r in rows if r[5].startswith("OK/OK"))
    a_rows = [r for r in rows if r[2] == "A_primary"]
    a_got = sum(1 for r in a_rows if r[5].startswith("OK/OK"))
    failed = sum(1 for r in rows if "FAILED" in r[5])
    print(f"records={len(rows)} readable={got} failed_fetch={failed}")
    print(f"JOB A primary cell: {a_got}/{len(a_rows)} readable")

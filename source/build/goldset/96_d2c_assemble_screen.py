#!/usr/bin/env python3
"""
96_d2c_assemble_screen.py — D.2.c (son preference), assemble the blinded screen.

Mirror of 96_a14 / C.2.h step 59. Reads every verdict file, rejoins the blinded metadata
(title/doi/year/source_channels) from the screen frame by id, and produces:
  - tiers (T1 RELEVANT / T2 UNCERTAIN / T3 NOT_RELEVANT),
  - the POOLING SET = verdict RELEVANT AND cell in PRIMARY AND evidence not review/theory, split by
    identification (natural-experiment or revealed-stopping = identified core vs ASSOCIATIONAL_ONLY),
...
# D.2.c's identified core: revealed-preference differential stopping (near-random sex of early births)
# and natural experiments off sex-selection-technology arrival / policy.
IDENTIFIED = {"NATURAL_EXPERIMENT", "REVEALED_STOPPING"}
...
def main():
    ...
    theory = [v for v in verdicts if C(v) in {"THEORY", "MECHANISM_PREFERENCE_ONLY"}]
    ...
    R = [f"# D.2.c blinded screen — assembly report", "",
         f"Frame {len(verdicts)} records ({n_with_abstract} with abstracts), "
         f"{len(manifest['manifest'])} batches. Assembled by `96_d2c_assemble_screen.py`.", "",
         "## Verdict tiers", "",
         ...
         f"  - identified core (natural-experiment or revealed-stopping): **{len(identified)}**",

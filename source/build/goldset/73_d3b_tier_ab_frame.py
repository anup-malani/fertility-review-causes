#!/usr/bin/env python3
"""
73_d3b_tier_ab_frame.py — D.3.b (climate anxiety / eco-doomerism), stage A4.

Build the two-tier gold FRAME from the A3 cold-start anchors (GACS channels 1 + 3). Direct mirror of
B.1's `65_b1_tier_ab_frame.py`; only the SLUG, cache namespace, and cell taxonomy change.

  Tier A  = the direct-empirical seeds (the anchors themselves), resolved in OpenAlex and enriched with
            abstracts. This is the primary empirical recall set. Theory-stream anchors, the REVERSE-cell
            anchor, and routing decoys are split out, NOT counted as Tier-A empirical.
...
Neither tier is frozen here. This is a candidate frame, mirroring child-labor step 57 and B.1 step 65.

FORWARD-SEED POLICY (inherited from B.1): forward-cite an anchor UNLESS it is a routing decoy OR
  (cited_by_count > FWD_THEORY_CAP AND provisional_cell == "THEORY"). Backward references from ALL
  resolved anchors. Excluded forward seeds are logged with counts for transparency. D.3.b's literature
  is small and recent, so the forward clouds are expected to be far smaller than B.1's (~4,900) — the
  cap is retained for safety, not because any theory anchor is expected to hit it.

LEAKAGE WALL (carried from A3): the PLOS Climate SR that fed channel-1 anchors here does NOT feed A6
  query terms. This step only builds the citation frame; it does not mine query vocabulary.
...
EMPIRICAL_CELLS = {"PRIMARY_HABITABILITY_FEAR", "PRIMARY_CARBON_ETHICS", "PRIMARY_ECO_PESSIMISM",
                   "DESIRE_INDEPENDENCE"}

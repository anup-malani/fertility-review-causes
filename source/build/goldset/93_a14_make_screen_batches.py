#!/usr/bin/env python3
"""
93_a14_make_screen_batches.py — A.14 (coital frequency and fecundability), stage-3 screen input.

Prepare the merged ~3,035-record screen frame (92) for blinded title/abstract LLM screening. No record
is filtered here (keep-and-route): pruning by vocabulary distance would bias recall. Records are
deterministically shuffled, stripped of discovery provenance (source_channels, cited_by_count, doi are
blinded), and split into fixed-size batches. A committed manifest records paths + SHA-256; batch
payloads live in temp/ (reproducible from the committed frame).

Mirror of C.2.h step 56 / A.3 step 91. SLUG, SEED, and the rubric (A.14's cells, its SIX walls, its
PM/FDT/SDT scope, and the reverse-causality + age-confound identification tags) differ. The routing
decoys must surface route-away: Bailey 2006 -> OFF_CONTRACEPTION; Hajnal 1965 -> OFF_UNION_FORMATION;
Kennedy-Rivera-McNeilly 1989 -> OFF_LACTATION; Menken-Trussell-Larsen 1986 -> OFF_FECUNDITY_CAPACITY;
Wilcox 1988 -> OFF_FETAL_LOSS.
...
RUBRIC = """# Blinded title/abstract screening rubric — coital frequency and fecundability (A.14) — v1

#!/usr/bin/env python3
"""
125_b7_tier_ab_frame.py — B.7, stage A4. Build the Tier A / Tier B citation frame.

Tier A is the verified anchor set from 116. Tier B is the orthogonal frame: everything the anchors
...
CACHE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "b7_frame_cache.json")
cache = json.load(open(CACHE)) if os.path.exists(CACHE) else {}

FORWARD_CAP = 1200          # per-seed budget control, NOT a judgement about the seed
PAGE = 200
...
# For B.7 the diagnostic is deliberately narrow: it asks what fraction of a seed's citation cloud
# carries a FERTILITY QUANTITY at all. That is the chapter's precision problem stated as a number. A
# broader list including "sexual dysfunction" would score the link-1 seeds near 100% and stop
# discriminating, which would make the diagnostic useless exactly where it is most needed.
...
def on_topic(rec):
    ...
    return any(t in blob for t in TOPIC_TERMS)
...
def main():
    ...
    for rec, w in seedinfo:
        ...
        is_empirical = cell in EMPIRICAL_CELLS
        ...
        log_rows.append(dict(title=rec["title"][:58], cell=cell, seed=sid, empirical=is_empirical,
                             ...
                             truncated=truncated, on_topic=frac))
        print(f"  {cell[:26]:<26} back={len(back):>4} fwd={len(fwd):>5}/{total or 0:<5} "
              f"on-topic={f'{frac:.1%}' if frac is not None else 'n/a':>6}  {rec['title'][:42]}")
    ...
    L = [f"# A4 Tier A / Tier B citation frame — {SLUG} (B.7)", "",
         f"**Tier A: {len(tier_a)} verified anchors** ({sum(1 for r in tier_a if r['provisional_cell'] in EMPIRICAL_CELLS)} "
         "empirical primary-cell, the causal recall denominator).", "",
         ...
         "## Per-seed yield", "",
         "On-topic fraction is a SEED-SELECTION DIAGNOSTIC computed after retrieval. It is never "
         "applied as a filter on the frame: filtering the forward fetch by topic vocabulary would "
         "prune Tier B by distance from the production query and make Recall(B) circular.", "",
    ...
    for r in sorted(log_rows, key=lambda x: -(x["n_fwd"])):
        ...
        L.append(f"| {r['title']} | `{r['cell']}` | {r['n_back']} | {r['n_fwd']} | "
                 f"{r['fwd_total'] or 0} | {'**yes**' if r['truncated'] else 'no'} | {ot} |")
    ...
    if trunc:
        ...
        for r in trunc:
            L.append(f"- **{r['title']}** (`{r['cell']}`): pulled {r['n_fwd']:,} of {r['fwd_total']:,} "
                     f"citing works, on-topic {r['on_topic']:.1%}.")
    else:

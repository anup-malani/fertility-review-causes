...
STUB_MARKERS = ("security verification", "just a moment", "cf-challenge", "enable javascript and cookies",
...
def read_html(path):
    """Return (blob, head, is_stub). Proxy downloads sometimes save the publisher's HTML full-text view
    (usable — DOI in citation_doi/og meta) or a Cloudflare/bot-challenge page (a dead stub to re-fetch)."""
    t = path.read_text(encoding="utf-8", errors="ignore")
    low = t[:4000].lower()
    is_stub = any(m in low for m in STUB_MARKERS) and "citation_doi" not in low and "og:title" not in low
    # Surface the machine-readable identity tags publishers embed, so DOI/title matching sees them first.
    tags = " ".join(re.findall(r'(?:citation_doi|og:title|dc\.identifier|citation_title)["\']?\s*'
                               r'content=["\'](.*?)["\']', t, re.I)[:6])
    head = re.sub(r"<[^>]+>", " ", t[:2000])
    return tags + " " + t, re.sub(r"\s+", " ", head)[:180], is_stub


def sniff(path):
    """True kind by magic bytes, since extensions lie (a .html that is really a PDF, or vice-versa)."""
    b = path.read_bytes()[:1024]
    if b[:5] == b"%PDF-":
        return "pdf"
    if b"<!doctype html" in b[:400].lower() or b"<html" in b[:400].lower():
        return "html"
...
def main():
    ...
    present_wids = {p.name.split("__")[0] for p in PDFDIR.glob("*") if WNAME_RE.match(p.name)}
    # Consider every non-directory drop, whatever its extension — batch tools save .pdf, .html, and
    # extensionless files; the real kind is sniffed from bytes, not trusted from the suffix.
    candidates = [p for p in sorted(PDFDIR.iterdir())
                  if p.is_file() and not WNAME_RE.match(p.name) and p.suffix.lower() != ".md"]
    newly, unmatched, dups, blocked, taken, batch_wids = [], [], [], [], set(), set()
    for src in candidates:
        kind = sniff(src)
        if kind == "html":
            blob, head, is_stub = read_html(src)
            if is_stub:                              # Cloudflare/bot wall -> failed download, re-fetch
                blocked.append(src.name)
                continue
            ext = ".html"
        else:
            blob, head = read_pdf(src)
            ext = ".pdf"
        pid, title, doi, how = identify(blob, by_doi, by_title)
        if not pid:
            unmatched.append((src.name, doi, head[:140]))
            continue
        if pid in present_wids or pid in batch_wids:  # already present / earlier in batch = duplicate
            dups.append((src.name, pid, doi))
            continue
        batch_wids.add(pid)
        newname = f"{pid}__{slug_title(title)}{ext}"
        newly.append((src.name, newname, pid, doi, f"{how}/{ext[1:]}", doi in targets if doi else False))
        if args.apply:
            dest = PDFDIR / newname
            if dest.exists() or newname in taken:
                dest = PDFDIR / f"{pid}__{slug_title(title)}-dup{ext}"
            src.rename(dest); taken.add(dest.name)
    ...
    R = [f"# B.1 PDF ingest reconciliation ({'APPLIED' if args.apply else 'DRY-RUN'})", "",
         ...
         f"- blocked/failed downloads (Cloudflare stub, re-fetch): **{len(blocked)}**",
    ...
    if blocked:
        R += ["", "## Blocked / failed downloads (Cloudflare 'Security verification' stub — re-fetch)", "",
              "These have no article content and no recoverable DOI; re-retrieve them (log into the "
              "UChicago proxy in-browser and download directly, or retry).", ""]
        R += [f"- `{n}`" for n in blocked]
    if unmatched:

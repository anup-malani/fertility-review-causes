#!/usr/bin/env python3
"""350 — C.3.d cold-start anchor resolution (TICK-083).

A DIRECT PORT of 336 (C.2.f), which is the newest copy of this resolver and carries every fix the
chain has accumulated: TICK-074's four, the three defects C.6.a found on *Birth and Fortune*, and
C.2.b's HTML-markup fix. Ported deliberately rather than re-derived.

**It imports the canonical fold rather than making a fourteenth hand-copy.** `source/lib/textnorm.py`
went onto `main` with TICK-074 on 2026-09-08. `_fold` here is `textnorm.norm` with ONE documented
addition: an HTML-markup strip that runs first. TICK-082 is open against `textnorm.norm()` for
exactly that gap -- OpenAlex stores some titles with markup, and stripping non-alphanumerics turns
`<i>` into the token `i`, which deflates Jaccard and breaks contiguous stem containment. When
TICK-082 lands, delete the wrapper here and call `norm` directly. `textnorm.selftest()` runs at
start-up: a guard that silently stops firing is worse than no guard.

**The candidate list is split into `control` and `hand`, and scored separately.** The 65 controls
were taken PROGRAMMATICALLY by script 349 from 348's free-seed harvest -- titles, years and DOIs
copied, never retyped (`never-hand-type-a-record-id`) -- so they demonstrably exist and a failure on
one is a broken resolver. The 17 hand anchors are the Tier-A canon written from knowledge of the
literature, and a failure there may be a ghost citation. Without the split a run of NO_RESULTS is
uninterpretable (`validate-a-null-detector-on-positives`).

**`direction` is carried on every candidate and is the point of the run.** Scope §2 rules that the
FORWARD arm (return -> fertility) is C.3.d's primary cell and the BACKWARD arm (family size -> child
outcomes) is mechanism evidence on link 2, never pooled with it. The hand canon is split 8 THEORY /
5 BACKWARD / 4 FORWARD deliberately. Resolving it and then reading citation counts per direction is
a direct test of §2 on the canon rather than on record counts -- the same role `outcome_is_fertility`
played on C.2.f.

**Book canon needs the FIRST-author gate.** Becker 1960 is a chapter in an NBER conference volume and
several canon items are monographs or chapters; a monograph resolves to its own review at Jaccard
1.00 unless first-author agreement is required (`book-canon-first-author`, `anchor-resolver-book-canon`).

Usage: python3 source/build/goldset/350_c3d_cold_start_anchors.py [--out PATH]
"""
...
import textnorm                                        # noqa: E402  (path set above)

OUT = Path(sys.argv[sys.argv.index("--out") + 1]) if "--out" in sys.argv else \
    ROOT / "literature/search-logs/quantity-quality-tradeoff-cold-start-anchors.json"
...
# OpenAlex stores some titles with HTML markup. `textnorm.norm` does not strip it -- that is
# TICK-082, open on `main` -- and stripping non-alphanumerics turns "<i>" into the token "i", which
# deflates Jaccard and breaks contiguous stem containment. So markup comes off FIRST and everything
# else is the canonical fold. Delete this wrapper when TICK-082 lands.
...
CAND = ROOT / "literature/search-logs/quantity-quality-tradeoff-anchor-candidates.json"
...
def main():
    textnorm.selftest()          # a guard that silently stops firing is worse than no guard
    out = []
    for c in CANDIDATES:
        r = resolve(c)
        out.append(r)
    ...
    # Scope §2 on the canon: resolve by DIRECTION and report citation weight per arm.
    print("\n--- scope §2: the canon by direction ---")
    for d in ("THEORY", "BACKWARD", "FORWARD", "UNKNOWN"):
        rows = [r for r in out if r.get("direction") == d]
        if not rows:
            continue
        ok = [r for r in rows if r["verdict"] in MATCHED]
        cites = [(r.get("top_candidate") or {}).get("cited_by") or 0 for r in ok]
        med = sorted(cites)[len(cites) // 2] if cites else 0
        print(f"  {d:9} {len(ok)}/{len(rows):>3} resolved   total cites {sum(cites):>7,}   "
              f"median {med:>6,}")
    ...
    L = ["# C.3.d cold-start anchors", "",
         ...
         "programmatically from records script 348 found in other chapters' pools, so they "
    ...
    L += ["", "## By direction — scope §2 on the canon", "",
          "Scope §2 rules the FORWARD arm (return → fertility) primary and the BACKWARD arm "
          "(family size → child outcomes) mechanism evidence on link 2, never pooled. Citation "
          "weight per arm is the direct read on which arm the field treats as the tradeoff.", "",
          "| direction | resolved | n | total cites | median cites |", "|---|---|---|---|---|"]
    for _d in ("THEORY", "BACKWARD", "FORWARD", "UNKNOWN"):
        _rows = [r for r in out if r.get("direction") == _d]
        if not _rows:
            continue
        _ok = [r for r in _rows if r["verdict"] in MATCHED]
        _c = [(r.get("top_candidate") or {}).get("cited_by") or 0 for r in _ok]
        _med = sorted(_c)[len(_c) // 2] if _c else 0
        L.append(f"| `{_d}` | {len(_ok)} | {len(_rows)} | {sum(_c):,} | {_med:,} |")
    ...
    for r, t in twins:
        ...
        L.append(f"| {r['title'][:44]} | {tc['year']} | {tc['cited_by']} | {t['year']} | "
                 f"{t['cited_by']} |")
    L.append("")
    (OUT.parent / "quantity-quality-tradeoff-cold-start-anchors-log.md").write_text("\n".join(L))
    print(f"\nwritten: {OUT}")

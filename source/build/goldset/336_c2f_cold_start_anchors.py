#!/usr/bin/env python3
"""336 — C.2.f cold-start anchor resolution (TICK-081).

A DIRECT PORT of 319 (C.2.b), which is the newest copy of this resolver and carries all four
TICK-074 fixes, the three defects C.6.a found on *Birth and Fortune*, and C.2.b's HTML-markup fix.
Ported deliberately rather than re-derived. It imports nothing from `main`'s twelve older copies.

**Uses the canonical fold.** TICK-074 merged `source/lib/textnorm.py` into `main` on 2026-09-08, so
`_fold` here is checked against it at start-up rather than being a thirteenth hand-copy.

**The candidate list is split into `control` and `hand`, and scored separately.** The 65 controls
were taken PROGRAMMATICALLY from script 335's harvest -- titles, years and DOIs copied, never
retyped (`never-hand-type-a-record-id`) -- so they demonstrably exist and a failure on one is a
broken resolver. The 14 hand candidates are the Tier-A canon written from knowledge of the
...
**`outcome_is_fertility` is carried on every candidate and is not decoration.** Scope §4 records
that link 1 of this chapter's conjunction -- inequality raising required investment per child -- has
a substantial literature that does not measure fertility, while the full chain has ~2 records. Nine
of the fourteen hand anchors are flagged `false` for exactly that reason. Resolving them and then
reading that column is the direct test of §4 on the canon rather than on record counts.

**Book canon needs the FIRST-author gate.** Hirsch, Frank, Schor and Veblen are monographs, and a
monograph resolves to its own review at Jaccard 1.00 unless first-author agreement is required
(`book-canon-first-author`, `anchor-resolver-book-canon`).

Usage: python3 source/build/goldset/336_c2f_cold_start_anchors.py [--out PATH]
"""
...
OUT = Path(sys.argv[sys.argv.index("--out") + 1]) if "--out" in sys.argv else \
    ROOT / "literature/search-logs/rising-inequality-and-status-competition-cold-start-anchors.json"
...
CAND = ROOT / "literature/search-logs/rising-inequality-and-status-competition-anchor-candidates.json"
...
def main():
    ...
    for r, t in twins:
        ...
        L.append(f"| {r['title'][:44]} | {tc['year']} | {tc['cited_by']} | {t['year']} | "
                 f"{t['cited_by']} |")
    L.append("")
    (OUT.parent / "rising-inequality-and-status-competition-cold-start-anchors-log.md").write_text("\n".join(L))
    print(f"\nwritten: {OUT}")

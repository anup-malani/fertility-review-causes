#!/usr/bin/env python3
"""
117_b5_tier_ab_frame.py — B.5, stage A4. Build the Tier A / Tier B citation frame.
...
CACHE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "b5_frame_cache.json")
...
TOPIC_TERMS = ("fertility", "birth", "births", "childbearing", "family size", "parity",
               "total fertility", "reproduc", "conception", "birth rate")
...
def main():
    ...
    L = [f"# A4 Tier A / Tier B citation frame — {SLUG} (B.5)", "",
         f"**Tier A: {len(tier_a)} verified anchors** ({sum(1 for r in tier_a if r['provisional_cell'] in EMPIRICAL_CELLS)} "
         "empirical primary-cell or replacement, the causal recall denominator).", "",

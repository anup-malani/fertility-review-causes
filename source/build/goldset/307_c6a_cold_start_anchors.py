#!/usr/bin/env python3
"""307 — C.6.a cold-start anchor resolution (TICK-078).

A DIRECT PORT of 275 (C.3.e), which is the only copy of this resolver carrying all four fixes
TICK-074 records; the twelve copies on `main` do not, and a new chapter that inherits one of those
inherits a title channel that has never worked. Ported deliberately rather than re-derived.

Two additions for this chapter.

**A DOI rung, ahead of the title rungs.** Eighteen of the thirty-one candidates were harvested from
other chapters' pools by script 305 and arrive carrying a DOI. A DOI is an exact key; putting a title
...
eighteen controls are records that demonstrably exist — another chapter retrieved them. A resolver
failure on those is a broken resolver. The thirteen hand-typed candidates are author-year-title
...
Usage: python3 source/build/goldset/307_c6a_cold_start_anchors.py [--out PATH]
"""
...
OUT = Path(sys.argv[sys.argv.index("--out") + 1]) if "--out" in sys.argv else \
    ROOT / "literature/search-logs/easterlin-relative-income-cold-start-anchors.json"
...
CAND = ROOT / "literature/search-logs/easterlin-relative-income-anchor-candidates.json"
...
def main():
    ...
    for c in CANDIDATES:
        ...
        print(f"{r['verdict']:30s} {c['source']:8s} {c['link']:7s} {c['year']} -> "

#!/usr/bin/env python3
"""
119_b5_make_screen_batches.py — B.5, stage D2. Prepare the semantic screen.
...
are the ones a screener will otherwise get wrong:
  ...
  * The screen assigns a ROUTING CELL only. It does NOT assign the estimand level
    (ACCOUNTING_SHARE vs BEHAVIORAL_NET), which is a modeling fact invisible in an abstract.
  * Where routing turns on Wall 2 (conception vs survival margin) or Wall 5 (maternal age) and the
    abstract does not name the margin, the verdict is ROUTING_DEFERRED_TO_FULLTEXT rather than a
    substantive OFF_* cell. An OFF_* label an abstract cannot support is a silent false negative, and
    on this hypothesis the OFF_* cells are where most of the corpus lands.

Output: source/build/goldset/b5_screen_batches/batch_NN.json
...
BATCH_DIR = os.path.join(HERE, "b5_screen_batches")
...
def main():
    ...
    L = [f"# Screening rubric — {SLUG} (B.5)", "",
         ...
         "**The screen assigns a routing cell only.** It does not assign the estimand level "
         "(`ACCOUNTING_SHARE` vs `BEHAVIORAL_NET`); that is a modeling fact that abstracts of "
         "decomposition papers rarely state, and it is set at full-text extraction.", "",
         "**Route on the margin the estimate bites at, not on the disease, exposure, or population.** "
         "B.5 owns survival conditional on conception. Infection, nutrition, and maternal health are "
         "B.5's drivers AND other chapters' treatments, so the organism or exposure never routes the "
         "paper; the measured outcome does.", "",
         "**Defer rather than guess.** Where routing turns on Wall 2 (conception vs survival margin) "
         "or Wall 5 (maternal age as the identifying variation) and the abstract does not name the "
         "margin, use `ROUTING_DEFERRED_TO_FULLTEXT`. An `OFF_*` label assigned on an abstract that "
         "could not support it is a silent false negative.", "",
    ...
    L += ["", "## Verdict convention", "",
          ...
          "- `NOT_RELEVANT` — an `OFF_*` cell, including the large clinical-management and non-human "
          "literatures.", "",

...
Carrying counts forward
-----------------------
`--reuse <earlier probe .json>` copies a count from an earlier run when the query is byte-identical
-- same pass, same axis, same outcome axis. Anything edited, and every row that is new, is queried
live. The daily OpenAlex allowance is roughly 100 requests, and a full run is ~85 of them, so a run
that exists only to settle two new candidates would otherwise spend the day's budget re-measuring 35
axes that moved less than 1% in six days.

The cache carries the count AND the date it was measured. Every row in the JSON and every block in
the markdown says which date its number comes from, because a number whose provenance is erased is
worse than one that is stale (`a-cache-must-not-invent-provenance`). The control is never cached: it
is the detector that says the probe still works, and a cached control detects nothing.

Usage: python3 source/build/goldset/304_candidate_frame_probe.py [--date YYYY-MM-DD] [--reuse FILE]
"""
import argparse
import json
import re
...
STARTED = {
    ...
    # Added 2026-09-09 (TICK-083): both drafted since the 09-03 run.
    "child-cost-direct", "rising-inequality-and-status-competition",
    # Added 2026-09-10 (TICK-086): TICK-083 claimed it 09-09 and the chapter is drafted.
    "quantity-quality-tradeoff",
    # Added 2026-09-13: TICK-086's own chapter was drafted 09-10, so the run that picked C.3.f
    # cannot rank it again.
    "wealth-flows-reversal",
}
...
NARROW = [
 ...
 # C.3.d left NARROW on 2026-09-10: TICK-083 claimed it and check_coverage() refuses a candidate
 # that is also STARTED, which is how a started chapter is stopped from being ranked twice.
 # C.3.f left NARROW on 2026-09-13: TICK-086 claimed it and drafted the chapter on 09-10.
...
WIDE = [
 ...
 # Added 2026-09-10 (TICK-086). Both sat in the pass-1-only block with a narrow count BELOW the
 # incumbent union frame (C.3.f, 1101), so the corollary that retires a candidate on its narrow
 # count alone does not reach them. Without these two rows the 09-10 ranking would again be a
 # subset reported as the whole set (`candidate-list-is-itself-a-filter`).
 ("C.4.a",'("Malthusian" OR "preventive check" OR "positive check" OR "population pressure" OR "land pressure" OR "carrying capacity" OR "land inheritance" OR "subsistence crisis")'),
 ("D.2.c",'("son preference" OR "sex preference" OR "gender preference" OR "sex ratio at birth" OR "sex-selective abortion" OR "daughter aversion" OR "differential stopping")'),
 # Added 2026-09-13. With C.3.f started, the 4th-placed union frame is D.2.c at 2024. Three
 # pass-1-only candidates have a NARROW count below that -- C.2.d 1301, C.2.e 1424, A.4 1852 --
 # so a lower bound alone cannot retire them from a top-four list, and the 09-10 ranking would be
 # a subset reported as the whole set for the third time (`candidate-list-is-itself-a-filter`).
 # Every other pass-1-only candidate is already above 2024 on its own lower bound.
 ("C.2.d",'("family policy" OR "family allowance" OR "birth grant" OR "maternity leave" OR "cash transfer" OR "fertility incentive" OR "pronatalist policy" OR "child subsidy")'),
 ("C.2.e",'("women employment" OR "maternal employment" OR "child penalty" OR "motherhood penalty" OR "opportunity cost" OR "value of time" OR "market work" OR "wage elasticity")'),
 ("A.4",  '("abortion" OR "termination of pregnancy" OR "pregnancy termination" OR "menstrual regulation" OR "abortion restriction" OR "abortion policy")'),
]
...
SUBLIT = [
 ...
 ("C.4.a", "core (pass 1)", '("land constraints" OR "land scarcity" OR "land availability")'),
 ("C.4.a", "Malthusian check vocabulary", '("Malthusian" OR "preventive check" OR "positive check")'),
 ("C.4.a", "population pressure / carrying capacity", '("population pressure" OR "land pressure" OR "carrying capacity")'),
 ("D.2.c", "core (pass 1)", '("son preference" OR "missing women")'),
 ("D.2.c", "sex selection technology", '("sex selection" OR "sex-selective abortion" OR "prenatal sex determination")'),
 ("D.2.c", "stopping-rule framing", '("differential stopping" OR "sex composition of children" OR "son-biased fertility")'),
 # Added 2026-09-13 alongside the three new WIDE rows.
 ("C.2.d", "core (pass 1)", '("child allowance" OR "baby bonus" OR "child benefit" OR "child tax credit")'),
 ("C.2.d", "leave policy", '("parental leave" OR "maternity leave" OR "paternity leave")'),
 ("C.2.d", "policy framing", '("family policy" OR "pronatalist policy" OR "fertility incentive" OR "birth grant")'),
 ("C.2.e", "core (pass 1)", '("opportunity cost of time" OR "female wage" OR "gender wage gap")'),
 ("C.2.e", "participation framing", '("female labor force participation" OR "female labour force participation" OR "women employment" OR "maternal employment")'),
 ("C.2.e", "child-penalty framing", '("child penalty" OR "motherhood penalty" OR "motherhood wage penalty")'),
 ("A.4", "core (pass 1)", '("induced abortion" OR "abortion legalization" OR "abortion legalisation")'),
 ("A.4", "access and law framing", '("abortion access" OR "abortion law" OR "abortion restriction" OR "abortion policy")'),
 ("A.4", "clinical vocabulary", '("termination of pregnancy" OR "pregnancy termination" OR "menstrual regulation")'),
]
...
UNION = [
 ...
 ("C.4.a", '("land constraints" OR "land scarcity" OR "land availability" OR "Malthusian" OR "preventive check" OR "positive check" OR "population pressure" OR "land pressure" OR "carrying capacity" OR "land inheritance" OR "subsistence crisis")'),
 ("D.2.c", '("son preference" OR "sex selection" OR "missing women" OR "sex preference" OR "gender preference" OR "sex ratio at birth" OR "sex-selective abortion" OR "daughter aversion" OR "differential stopping")'),
 # Added 2026-09-13.
 ("C.2.d", '("child allowance" OR "baby bonus" OR "child benefit" OR "child tax credit" OR "parental leave" OR "maternity leave" OR "paternity leave" OR "family allowance" OR "family policy" OR "pronatalist policy" OR "fertility incentive" OR "birth grant" OR "child subsidy")'),
 ("C.2.e", '("opportunity cost of time" OR "female wage" OR "gender wage gap" OR "female labor force participation" OR "female labour force participation" OR "women employment" OR "maternal employment" OR "child penalty" OR "motherhood penalty" OR "value of time" OR "market work")'),
 ("A.4",   '("induced abortion" OR "abortion legalization" OR "abortion legalisation" OR "abortion access" OR "abortion law" OR "abortion restriction" OR "abortion policy" OR "termination of pregnancy" OR "pregnancy termination" OR "menstrual regulation")'),
]
...
HOMONYM = [
 ...
 # Added 2026-09-10 (TICK-086), for the three candidates that can win this ranking. The 09-09 run
 # printed this heading over an empty table because every row belonged to a started chapter; an
 # empty homonym table is the absence of a query, not the absence of a homonym.
 ("C.3.f", "intergenerational transfer, unrestricted", '"intergenerational transfer"', None),
 ("C.3.f", "intergenerational transfer INTERSECT bequests and estates",
  '("intergenerational transfer") AND ("bequest" OR "inheritance tax" OR "estate" OR "wealth tax")', None),
 ("C.3.f", "intergenerational transfer INTERSECT fertility", '"intergenerational transfer"', OUTCOME_WIDE),
 ("C.3.f", "wealth flows, unrestricted", '"wealth flows"', None),
 ("C.3.f", "wealth flows INTERSECT capital markets",
  '("wealth flows") AND ("portfolio" OR "capital flows" OR "asset prices" OR "investors")', None),
 ("C.3.f", "boundary: C.3.c old-age security (a written chapter)",
  '("wealth flows" OR "intergenerational transfer" OR "old age support" OR "filial support") AND ("pension" OR "social security" OR "public transfer")', OUTCOME_WIDE),
 ("C.3.f", "boundary: D.1.b westernization (a drafted chapter)",
  '("wealth flows" OR "intergenerational transfer") AND ("westernization" OR "westernisation" OR "mass schooling" OR "developmental idealism")', OUTCOME_WIDE),
 ("C.3.f", "boundary: C.3.a mode of production (an unstarted candidate)",
  '("wealth flows" OR "value of children" OR "child labour contribution" OR "child labor contribution") AND ("agricultural household" OR "peasant household" OR "farm household" OR "subsistence agriculture")', OUTCOME_WIDE),
 ("C.3.f", "bequest residue INSIDE the fertility-restricted frame",
  '("wealth flows" OR "intergenerational transfer" OR "intergenerational wealth") AND ("bequest" OR "inheritance tax" OR "estate" OR "wealth tax")', OUTCOME_WIDE),
 ("C.3.f", "boundary: C.2.b price of children (a drafted chapter)",
  '("wealth flows" OR "intergenerational transfer" OR "value of children" OR "old age support") AND ("cost of children" OR "child expenditure" OR "price of children" OR "cost of raising children")', OUTCOME_WIDE),
 ("C.3.f", "value-of-children vocabulary alone", '("value of children" OR "children economic value")', OUTCOME_WIDE),
 ("C.4.a", "carrying capacity, unrestricted", '"carrying capacity"', None),
 ("C.4.a", "carrying capacity INTERSECT ecology and wildlife",
  '("carrying capacity") AND ("habitat" OR "wildlife" OR "grazing" OR "ecosystem")', None),
 ("C.4.a", "Malthusian INTERSECT fertility", '"Malthusian"', OUTCOME_WIDE),
 ("C.4.a", "boundary: A.7 age at marriage (an unstarted candidate)",
  '("Malthusian" OR "preventive check" OR "land scarcity") AND ("age at marriage" OR "marriage timing" OR "European marriage pattern")', OUTCOME_WIDE),
 ("D.2.c", "sex selection, unrestricted", '"sex selection"', None),
 ("D.2.c", "sex selection INTERSECT animal breeding",
  '("sex selection" OR "sexed semen") AND ("cattle" OR "bovine" OR "dairy" OR "livestock" OR "poultry")', None),
 ("D.2.c", "son preference INTERSECT fertility", '"son preference"', OUTCOME_WIDE),
 ("D.2.c", "boundary: A.10 sex-ratio marriage market (a started chapter)",
  '("son preference" OR "sex selection" OR "missing women") AND ("marriage market" OR "marriage squeeze" OR "bride price" OR "surplus men")', OUTCOME_WIDE),
 # Added 2026-09-13 for the three top-ranked candidates that had no rows. A.6 and A.3 are both
 # residual categories carved out of neighbours that are themselves unstarted, so the boundary
 # rows below matter more here than the homonym rows: they measure inherited scoping debt, which
 # is what settles a tie that the union frame cannot (see the C.6.a pick).
 ("A.6", "stigma, unrestricted", '"stigma"', None),
 ("A.6", "abortion stigma, unrestricted", '"abortion stigma"', None),
 ("A.6", "stigma frame INTERSECT miscarriage vocabulary (the B.5 boundary and the clinical homonym)",
  '("stigma" OR "taboo") AND ("spontaneous abortion" OR "miscarriage" OR "pregnancy loss")', OUTCOME_WIDE),
 ("A.6", "boundary: A.4 induced-abortion access (an unstarted candidate)",
  '("stigma" OR "taboo" OR "social disapproval") AND ("abortion law" OR "abortion access" OR "abortion legalization" OR "abortion legalisation")', OUTCOME_WIDE),
 ("A.6", "boundary: A.5 family-planning programs (an unstarted candidate)",
  '("stigma" OR "taboo" OR "social disapproval") AND ("family planning program" OR "family planning programme" OR "family planning services" OR "unmet need for contraception")', OUTCOME_WIDE),
 ("A.6", "boundary: A.2 contraceptive technology (an unstarted candidate)",
  '("stigma" OR "taboo" OR "social disapproval") AND ("oral contraceptive" OR "contraceptive technology" OR "intrauterine device" OR "modern contraception")', OUTCOME_WIDE),
 ("A.3", "social learning, unrestricted", '"social learning"', None),
 ("A.3", "social learning INTERSECT machine learning and animal behaviour",
  '("social learning") AND ("reinforcement learning" OR "neural network" OR "foraging" OR "primates" OR "songbirds")', None),
 ("A.3", "diffusion INTERSECT the physical sciences",
  '("diffusion") AND ("molecular" OR "membrane" OR "solute" OR "porous media")', None),
 ("A.3", "boundary: A.20 cultural diffusion mechanisms (an unstarted candidate)",
  '("diffusion of fertility control" OR "fertility diffusion" OR "spread of birth control" OR "family limitation") AND ("social network" OR "peer effect" OR "social influence" OR "social contagion")', OUTCOME_WIDE),
 ("A.3", "boundary: D.1.a postmaterialism and secularization (a drafted chapter)",
  '("ideational change" OR "diffusion of fertility control") AND ("secularization" OR "secularisation" OR "individualism" OR "postmaterialism" OR "value change")', OUTCOME_WIDE),
 ("A.3", "boundary: A.2 contraceptive technology (an unstarted candidate)",
  '("diffusion" OR "innovation diffusion" OR "spread of birth control") AND ("oral contraceptive" OR "contraceptive technology" OR "intrauterine device" OR "modern contraception")', OUTCOME_WIDE),
 ("C.3.a", "mode of production, unrestricted", '"mode of production"', None),
 ("C.3.a", "mode of production INTERSECT fertility", '"mode of production"', OUTCOME_WIDE),
 ("C.3.a", "farm-household vocabulary INTERSECT farm labour supply rather than fertility",
  '("farm household" OR "peasant household" OR "agricultural household") AND ("labor supply" OR "labour supply" OR "crop yield" OR "productivity")', None),
 ("C.3.a", "boundary: C.3.b child labour and compulsory schooling (a written chapter)",
  '("agricultural household" OR "peasant household" OR "farm household" OR "mode of production") AND ("child labour" OR "child labor" OR "compulsory schooling" OR "school enrollment")', OUTCOME_WIDE),
 ("C.3.a", "boundary: C.3.f wealth flows (a drafted chapter)",
  '("agricultural household" OR "peasant household" OR "farm household" OR "mode of production") AND ("wealth flows" OR "value of children" OR "old age support" OR "intergenerational transfer")', OUTCOME_WIDE),
 ("C.3.a", "boundary: C.2.g urbanization (an unstarted candidate)",
  '("agricultural household" OR "peasant household" OR "farm household" OR "agrarian society") AND ("urbanization" OR "urbanisation" OR "rural-urban migration" OR "urban residence")', OUTCOME_WIDE),
]
...
def load_reuse(path):
    """(pass, axis, outcome) -> (n, date) from an earlier probe JSON. Missing file is fatal."""
    blob = json.loads(Path(path).read_text())
    when = blob["date"]  # fallback only: probes written before rows carried their own stamp
    wide = blob.get("outcome_axis_wide")
    narrow = blob.get("outcome_axis")
    cache = {}
    for key, outcome in (("pass1_narrow", narrow), ("pass2_wide", narrow),
                         ("pass3_sublit", wide), ("pass4_union", wide)):
        for r in blob.get(key, []):
            if r.get("n") is not None:
                cache[(key, r["axis"], outcome)] = (r["n"], r.get("measured_on", when))
    for r in blob.get("homonym", []):
        if r.get("n") is not None:
            cache[("homonym", r["axis"], r.get("outcome"))] = (r["n"], r.get("measured_on", when))
    dates = sorted({d for _, d in cache.values()})
    print(f"REUSE: {len(cache)} counts from {path}, first measured {', '.join(dates)}; "
          "any edited axis re-queries.")
    return cache


def run(rows, outcome, pass_name, cache):
    out = []
    for row in rows:
        axis = row[-1]
        hit = cache.get((pass_name, axis, outcome))
        if hit:
            n, err, when = hit[0], None, hit[1]
        else:
            n, err = count(axis, outcome)
            when = TODAY
        out.append({"row": row[:-1], "axis": axis, "n": n, "err": err, "measured_on": when})
        label = " / ".join(str(x) for x in row[:-1])
        mark = "" if when == TODAY else f"  (carried from {when})"
        print(f"  {label:52} {n!s:>8}  {err or ''}{mark}", flush=True)
        if not hit:
            time.sleep(0.35)
    return out
...
def main():
    ...
    ap.add_argument("--dry-run", action="store_true",
                    help="print which rows would be queried live and which would be carried "
                         "forward, and make no API call. A cache that silently misses spends the "
                         "day's budget, so check the plan before spending it.")
    ap.add_argument("--reuse", default=None,
                    help="earlier probe .json; byte-identical queries are carried forward with "
                         "their original measurement date rather than re-queried")
    args = ap.parse_args()
    TODAY = args.date
    cache = load_reuse(args.reuse) if args.reuse else {}
    ...
    plan = [("pass1_narrow", NARROW, OUTCOME), ("pass2_wide", keep(WIDE), OUTCOME),
            ("pass3_sublit", keep(SUBLIT), OUTCOME_WIDE), ("pass4_union", keep(UNION), OUTCOME_WIDE)]
    if args.dry_run:
        total_live = 1  # the control is always live
        for name, rows, outcome in plan:
            miss = [r for r in rows if (name, r[-1], outcome) not in cache]
            total_live += len(miss)
            print(f"{name:14} {len(rows) - len(miss):3} carried  {len(miss):3} live")
            for r in miss:
                print(f"      LIVE  {' / '.join(str(x) for x in r[:-1])}")
        hmiss = [r for r in keep(HOMONYM) if ("homonym", r[2], r[3]) not in cache]
        total_live += len(hmiss)
        print(f"{'homonym':14} {len(keep(HOMONYM)) - len(hmiss):3} carried  {len(hmiss):3} live")
        for r in hmiss:
            print(f"      LIVE  {r[0]} / {r[1]}")
        print(f"\nTOTAL live requests: {total_live} (control included). Daily allowance ~100.")
        return
    ...
    p1 = run(NARROW, OUTCOME, "pass1_narrow", cache)
    print("\nPASS 2 — second vocabulary, the small end of pass 1:")
    p2 = run(keep(WIDE), OUTCOME, "pass2_wide", cache)
    print("\nPASS 3 — sub-literatures of the finalists, each alone:")
    p3 = run(keep(SUBLIT), OUTCOME_WIDE, "pass3_sublit", cache)
    print("\nPASS 4 — deduplicated union frame, the finalists:")
    p4 = run(keep(UNION), OUTCOME_WIDE, "pass4_union", cache)
    print("\nHOMONYM — shared vocabulary against an unrelated literature:")
    hm_rows = keep(HOMONYM)
    if not hm_rows:
        print("  ! ZERO ROWS — every HOMONYM entry belongs to a chapter that has since started.")
        print("    This pass is per-candidate and is not maintained forward. Add rows for the")
        print("    candidate you are about to choose; an empty table is not a clean homonym check.")
    hm = []
    for code, label, axis, outcome in hm_rows:
        hit = cache.get(("homonym", axis, outcome))
        if hit:
            n, err, when = hit[0], None, hit[1]
        else:
            n, err = count(axis, outcome)
        ...
        hm.append({"row": [code, label], "axis": axis, "outcome": outcome, "n": n, "err": err,
                   "measured_on": when})
        mark = "" if when == TODAY else f"  (carried from {when})"
        print(f"  {code} / {label:46} {n!s:>8}  {err or ''}{mark}")
        if not hit:
            time.sleep(0.35)

    all_rows = p1 + p2 + p3 + p4 + hm
    live = sum(1 for r in all_rows if r["measured_on"] == TODAY)
    carried = sorted({r["measured_on"] for r in all_rows if r["measured_on"] != TODAY})
    print(f"\nPROVENANCE: {live} of {len(all_rows)} counts measured today; "
          f"{len(all_rows) - live} carried forward from {', '.join(carried) or '—'}.")

    blob = {"date": args.date, "reuse_source": args.reuse,
            "outcome_axis": OUTCOME, "outcome_axis_wide": OUTCOME_WIDE,
            "control": {"label": CONTROL[0], "n": ctrl, "measured_on": TODAY},
    ...
    lines = [f"# Candidate frame probe — {args.date}", "",
             ...
             (f"**Provenance: {live} of {len(all_rows)} counts were measured on {args.date}; "
              f"{len(all_rows) - live} are carried forward from "
              f"{', '.join(carried) or 'nothing'}** — a count is reused only when its pass, its "
              "axis and its outcome axis are byte-identical to the earlier run, so every edited or "
              "new query below is live. The `measured` column says which is which."
              if carried else
              f"**Provenance: all {len(all_rows)} counts were measured on {args.date}.**"), "",
             ...
             "|---|---|---|---|---|---|"]
    u_when = {r["row"][0]: r["measured_on"] for r in p4}
    ...
    for r in sorted((r for r in p1 if r["row"][0] not in union),
        ...
        c = r["row"][0]
        lines.append(f"| {c} | `{slug[c]}` | {r['n']} | {wide.get(c, '—')} | {r['measured_on']} |")
    ...
    if hm:
        ...
        lines += ["**Not run — zero rows.** Every entry in `HOMONYM` belongs to a hypothesis that has",

...
After 5b, 5c and 5e, 101 of 436 are on disk. This script decides what the rest are, and the
...
def main():
    ...
    L = [f"# Stage 5d retrieval handoff — {SLUG} (A.23)", "",
         ...
         f"**{len(have)} of {len(meta)} records are on disk ({pc(len(have), len(meta))})** after "
         "the scripted fetch (5b), the PMC text recovery (5c) and the free-twin pass (5e). "
         f"**{len(rows)} are not**, and they are split below by what would fix them rather than by "
         "how they failed. Re-run this script after any later retrieval pass — it reads the "
         "directory, so its counts follow whatever is actually on disk.", "",

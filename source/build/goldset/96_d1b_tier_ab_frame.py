#!/usr/bin/env python3
"""
96_d1b_tier_ab_frame.py — D.1.b (cultural westernization / developmental idealism), stage A4.

Build the two-tier gold FRAME from the A3 cold-start anchors (GACS channels 1 + 3). Mirror of
`73_d3b_tier_ab_frame.py` and B.1's `65_b1_tier_ab_frame.py`; the SLUG, cache namespace, and cell
taxonomy change, and the forward-citation policy is retuned for a much larger literature.

  Tier A  = the direct-empirical seeds (the anchors themselves), resolved in OpenAlex and enriched with
            abstracts. This is the primary empirical recall set. Theory-canon anchors and routing decoys
            are split out and NOT counted as Tier-A empirical.
...
FORWARD-SEED POLICY, retuned. The rule is inherited — forward-cite an anchor UNLESS it is a routing
  decoy OR (cited_by_count > FWD_THEORY_CAP AND provisional_cell is the theory cell) — but the
  parameters are not, because D.1.b's citation economics are the opposite of D.3.b's.

  D.3.b was a five-year-old literature whose forward clouds were small; its cap was retained "for
  safety, not because any theory anchor is expected to hit it". D.1.b is a fifty-year-old literature
  whose canon is among the most-cited in demography: Caldwell 1976 and 1980 together carry thousands
  of citations, and the two media quasi-experiments (La Ferrara ~1,450, Jensen & Oster ~790) sit inside
  development economics, so their forward clouds are broad rather than topical. Left at D.3.b's
  settings (20 pages, cap 1,500) this step would request on the order of 50,000 records and hand the
  screen a corpus dominated by papers that cite Caldwell as a throat-clearing reference.

  So: MAX_FORWARD_PAGES drops 20 -> 10 and FWD_THEORY_CAP drops 1,500 -> 600, which excludes the broad
  canon from forward seeding while keeping every empirical anchor. Backward references are still taken
  from ALL resolved anchors, including the excluded theory ones — backward is cheap, bounded by the
  reference list, and is where a fifty-year-old canon actually pays off. Excluded forward seeds are
  logged with their counts, because a silent cap reads as "we covered everything" when it did not.

  This is a budget parameter, not a recall claim. If the resulting frame is thin in a primary cell, the
  right response is to raise the cap for that cell's seeds and re-run, not to accept the thinness.

LEAKAGE WALL (carried from A3): no query vocabulary is mined here. This step only builds the citation
  frame.
...
PER_PAGE = 200
SLEEP = 0.35
MAX_FORWARD_PAGES = 10          # graceful hard cap per seed (2,000 works)
FWD_THEORY_CAP = 600            # THEORY anchors above this are not forward-seeded
RESOLVE_SIM_MIN = 0.50

THEORY_CELL = "DI_THEORY"
# D.3.b empirical cells: the three primary fear-content cells + the value-added desire-independence cell.
# REVERSE (reverse-causality design) is in-scope but context, NOT a Tier-A empirical seed; it is still
# backward/forward-cited into the frame because its citation neighborhood is on-topic. OFF_* cells are
# routing decoys, excluded by the ROUTING_DECOY family test below.
EMPIRICAL_CELLS = {"PRIMARY_DI_BELIEF", "PRIMARY_SCHOOLING_IDEATIONAL",
...
def resolve(anchor, refresh=False):
    ...
    # floor. OpenAlex sometimes ingests a section header into the title or drops a subtitle, which would
    # wrongly sink a correct DOI match below the floor. (Two D.3.b anchors are low-J book/subtitle cases
    # — Conly J=0.222, Helm J=0.214 — that resolve correctly only via this DOI-first path.)
    if anchor.get("doi"):
        try:
...
def main():
    ...
    L += ["", "## Next gate", "",
          "Screen the whole Tier-B frame with the D.3.b topical/estimand rubric (routing on the three "
          "frozen boundary walls: vs D.1.a postmaterialism, vs D.3.a clinical anxiety, vs C.5.a economic "
          "uncertainty). Do NOT prune the frame by vocabulary distance from the future production query — "
          "that would bias Recall(B). The routing decoys (Lesthaeghe D.1.a, Adsera C.5.a, physical "
          "climate-shock, biological repro-health SR) must surface as route-away at screen."]

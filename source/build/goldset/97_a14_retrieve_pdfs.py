#!/usr/bin/env python3
"""TICK-091 stage 5: retrieve open-access PDFs for the A.14 estimand-ready pooling set.

Mirrors 82_d3b_retrieve_pdfs.py (OpenAlex all-locations + Unpaywall + Europe PMC + derived landing-page
constructions, %PDF magic-byte verification, idempotent). A.14 pool from the assembler
...
Outputs:
  literature/pdfs/coital-frequency-biological/W<id>__<slug>.pdf   (gitignored if large)
  extraction/coital-frequency-biological-pdf-retrieval-log.csv
  extraction/coital-frequency-biological-missing-pdf-dois.csv     (human handoff)
"""
...
def load_pool() -> list[dict]:
    ...
    for rec in pool:
        ...
        rec["priority"] = (PRIORITY_IDENTIFIED if ident in ("EXOGENOUS_FREQUENCY_SHOCK", "PROSPECTIVE_MEASUREMENT")

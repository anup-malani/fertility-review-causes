#!/usr/bin/env python3
"""
91_a14_citation_frame.py — A.14 (coital frequency and fecundability), stage-3 Tier-A/Tier-B frame.
...
Design decisions specific to A.14
-----------------------------------
  * FORWARD-SEED THE EMPIRICAL CORE ONLY. The six registered-wall decoys (A.2-A.6 contraception,
    A.7/C.7 marriage, A.13 lactation, A.15 fecundity capacity, B.5 fetal loss) and the proximate-
    determinant theory canon (Bongaarts 1978, Davis & Blake 1956, Wood 1989) are held OUT of forward-
    seeding — their forward-citation sets are enormous and off-estimand (Bongaarts is cited ~thousands).
    They still contribute BACKWARD references for context, and the decoys are retained so the screen is
    tested on routing. Forward-seeds: the biometric fecundability studies (Barrett-Marshall, Wilcox
    1995), the separation/abstinence naturals (Caldwell-Caldwell, Lindstrom-Saucedo), and the
    sex-recession trend studies (Twenge 2017, Ueda 2020).
  * RESOLVE TO THE VERSION OF RECORD, NOT THE REPRINT. Two anchors verified to a 2017 reprint DOI
    (Davis & Blake, Hajnal — the version-drift rows). Anchors are resolved by TITLE search sorted by
    cited_by_count and matched on Jaccard, which lands the most-cited version regardless of which DOI
    Crossref returned.
...
mined as production-query terms for A.14.

Outputs:
  literature/search-logs/coital-frequency-biological-anchor-resolution.json
  literature/search-logs/coital-frequency-biological-tier-a.json
  literature/search-logs/coital-frequency-biological-tier-b-frame.json
  literature/search-logs/coital-frequency-biological-tier-ab-log.md
"""
...
def main():
    ...
    # Forward-seed the empirical core only. The proximate-determinant theory canon (Bongaarts,
    # Davis & Blake, Wood) and the six wall decoys are held OUT of forward-seeding — their forward sets
    # are enormous and off-estimand (Bongaarts 1978 is cited ~thousands) — but still contribute
    # backward references and routing tests.
    forward_families = {"biometric-fecundability", "abstinence-natural",
                        "separation-natural", "frequency-trend"}
    ...
    L = [f"# A.14 Tier-A/Tier-B citation frame — build log", "",

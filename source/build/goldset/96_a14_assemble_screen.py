#!/usr/bin/env python3
"""
96_a14_assemble_screen.py — A.14 (coital frequency and fecundability), assemble the blinded screen.

Mirror of C.2.h step 59. Reads every verdict file, rejoins the blinded metadata (title/doi/year/
source_channels) from the screen frame by id, and produces:
  ...
  - the POOLING SET = verdict RELEVANT AND cell in PRIMARY AND evidence not review/theory,
    split by identification (freq-shock or prospective = identified core vs ASSOCIATIONAL_ONLY),
...
verdict file (run 94 --audit first).
...
# A.14's identified core is two clean designs: exogenous frequency shocks (separation/abstinence
# naturals) and prospective measurement (daily-diary fecundability studies).
IDENTIFIED = {"EXOGENOUS_FREQUENCY_SHOCK", "PROSPECTIVE_MEASUREMENT"}


def load_validator():
    path = HERE / "94_a14_validate_screen.py"
    spec = importlib.util.spec_from_file_location("a14_screen_validator", path)
...
def main():
    ...
    theory = [v for v in verdicts if C(v) in {"THEORY", "MECHANISM_FREQ_ONLY"}]
    ...
    R = [f"# A.14 blinded screen — assembly report", "",
         f"Frame {len(verdicts)} records ({n_with_abstract} with abstracts), "
         f"{len(manifest['manifest'])} batches. Assembled by `96_a14_assemble_screen.py`.", "",
         "## Verdict tiers", "",
         ...
         f"  - identified core (freq-shock or prospective): **{len(identified)}**",
